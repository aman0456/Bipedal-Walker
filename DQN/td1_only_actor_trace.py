...
class Environment():
	...
	def train_in_env(self,render=False):
		...
		for ep_num in pbar:
			...
			# I_vec=torch.tensor(I_vec).to(DEVICE)
			adv=return_vals-state_values
			# import pdb
			# pdb.set_trace()
			actor_loss=-torch.mean(actor_traces*(adv.detach()))

			# critic_trace=critic_trace+self.critic_trace_decay*self.gamma + torch.mean(self.critic_loss_func(state_values,return_vals)*I_vec).type(torch.float32)
			critic_loss=self.critic_loss_func(state_values,return_vals).type(torch.float32)
			# critic_loss=critic_trace
